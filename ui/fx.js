...
// --- PATCH: robust wrong-answer shake/flash (capture phase) ---
(function(){
  function closestTarget(e){
    return e.target.closest('.game-card, .option-btn, [data-correct]') || e.target;
  }
  document.addEventListener('click', function(e){
    const b = e.target.closest('[data-correct]');
    if(!b) return;
    if (b.getAttribute('data-correct') === '0'){
      // play wrong sfx if available
      if (window.KPSfx && typeof KPSfx.wrong === 'function') { try{ KPSfx.wrong(); }catch(_){} }
      // shake the button itself
      const tgt = closestTarget(e);
      tgt.classList.remove('shake'); // reset if queued
      void tgt.offsetWidth; // reflow
      tgt.classList.add('shake');
      setTimeout(()=>tgt.classList.remove('shake'), 450);
      // red flash overlay
  ...
  }, true); // capture phase to ensure it always runs
})();
